...
HOLD_1 - 3 = 200ms Pause, um mehrfach erkannte Würfe auszuschliessen für jeden Wurf
...
def fade(strip,sourceColour,targetColour,duration):
    step_ms = 50.0/1000.0
...
def allowedToThrow(strip,flashIterations):
    ...
    fade(strip,Color(0,255,0),Color(6,6,6),1)    
        


def playerHitTheTarget(strip):
    fade(strip,Color(60,60,0),Color(6,6,6),0.2)

def playerMissedTheTarget(strip):
    fade(strip,Color(100,0,0),Color(40,0,0),0.2)
    fade(strip,Color(40,0,0),Color(6,6,6),0.2)
    
def trimColor(oldColor):
    if oldColor.r>255:
        newColor.r = 255 # type: ignore
    if oldColor.g>255:
        newColor.g = 255 # type: ignore
    if oldColor.b>255:
        newColor.b = 255 # type: ignore
    
    return newColor # type: ignore
...
def sendThrow(modifier: int, value: int):
	...
	if state == 'HOLD_1' or state == 'HOLD_2' or state == 'HOLD_3': #Wenn wir in der Pause sind
		end_time = time.time()	
		global start_time
