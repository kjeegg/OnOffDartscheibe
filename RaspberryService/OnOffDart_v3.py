...
'''
CHECK_GAME_UPDATE_INTERVAL_MAX = 7
CHECK_GAME_UPDATE_INTERVAL_MIN = 2

CHECK_GLOBAL_GAME_INTERVAL = 30 #Prüft alle X Sekunden ob es ein neues Spiel gibt
...
LOKAL_PLAYER:Spieler = None #Repräsentiert den lokalen Spieler (als Spieler objekt)
...
'''
def fetchCurrentGame(gamesJSON: json) -> json:
	max_uid = -1
...
def fetchCurrentGame() -> Spiel:
	...
	if(js != None):
		curGameJSON = fetchCurrentGame(js)#Ermittle die JSON des aktuellen Spiels
		if(curGameJSON != None):					
...
def setPlayer(player: Spieler):
	...
	LOKAL_PLAYER = player
	LOKAL_PLAYER_LOCK.release()
	logger.log(f"Neuer Lokaler Spieler gesezt: UID:{player.uid}, Name:{player.name}")
	return 
...
'''
def createPlayer(spiel: Spiel, arrayID:int)
	...
	logger.debug(f"Der Aktive Spieler({spieler.name}/{spieler.nickname}) des übergegebenen Spiels({spiel.uid}) wurde erfolgreich erzeugt")
	return spieler
...
@return Spieler das Spielerobjekt
'''
def getAktiveSpieler(spiel: Spiel) -> Spieler:
	aktivNummer:int = int(spiel.ActivePlayer)
	createPlayer(spiel, aktivNummer)
...
##############################################################################
###																		   ###
###				!!!! ALTER NOCH ZU ÜBERARBEITENDER CODE !!!!			   ###
###																		   ###
##############################################################################
...
##############################################################################
###																		   ###
###		    !!!! ENDE ALTER NOCH ZU ÜBERARBEITENDER CODE !!!!		   	   ###
###																		   ###
##############################################################################
...
def checkGameStateChanges(latestGame: Spiel):
	...
	if curState == 'INIT':
		if len(latestGame.UndoLog) > 1: #Selber noch keinen wurf getätigt, dafür aber der Gegner
			arrayID:int = 0 if int(spiel.ActivePlayer)==1 else 1#Wenn Aktiver Spieler 1, dann aktuell 0, sonst andersrum
			aktuellerSpieler:Spieler = createPlayer(latestGame, arrayID)
			setPlayer(aktuellerSpieler)
			logger.info(f"Der andere Spieler hat zu erst geworfen. Lokaler Spieler: ID{aktuellerSpieler.uid}, Name:{aktuellerSpieler.name}")
			setState('OTHER_PLAYER')
			return
		else: #Warte das ein Spieler endlich wirft
			return
	

	if curState == 'UEBERTRITT': #Passiert nix
		return
	...
	if curState == 'OTHER_PLAYER':
		aktivSpieler:Spieler = getAktiveSpieler(latestGame) #Prüfe wer laut neustem stand der aktive Spieler ist
		if aktivSpieler.uid == getPlayer().uid: #Aktiver Spieler sind wieder wir (aka anderer Spieler ist fertig)
...
def checkForNewGame():
	...
	while True:
		fetchedUID:int = int(fetchCurrentGamesUID())
		if fetchUIDGame != getUID(): #neues spiel wurde erstellt
			setUID(fetchedUID)
			setPlayer(None)
			setState('INIT')
...
def fetchGameUpdates():
	...
	while True:
		if(getState != 'NG')
			if getState() == 'INIT' or getState() == 'OTHER_PLAYER' or getState() == 'UEBERTRITT':
